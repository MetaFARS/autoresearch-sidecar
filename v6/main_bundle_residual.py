...
from autoresearch_bundle import KappaClient, REQUIRED_WORLD_METHODS, describe_program, run_workflow
...
async def run_research_loop(max_iterations: int, init_code_path: str, gpu_ids: list[int], debug: bool = False) -> None:
    print(describe_program(), flush=True)
    print(f"specialized world boundary: {list(REQUIRED_WORLD_METHODS)}", flush=True)
    ...
    for index in range(max_iterations):
        ...
        await run_workflow(world, client, debug=debug)
