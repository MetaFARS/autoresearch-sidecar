...
def emit_python_kernel(program: CompiledProgram, module_name: str = "compiled_kernel") -> str:
    ...
    sections = [
        "from __future__ import annotations\n\n",
        f'"""Task-family residual kernel with trace instrumentation emitted for {program.name}.\n\n'
...
def render_role_function(role: CompiledRole) -> str:
    lines = [
        f"async def run_{role.name}_role(",
        "    client: KappaClient,",
        "    tool_host: ToolHost,",
...
def render_step(step: CompiledStep, indent: str) -> list[str]:
    ...
    if isinstance(step, CompiledRoleStep):
        ...
        lines = [
            f"{indent}{step.save_as} = await run_{step.role_name}_role(client, tool_host, {binding_expr}, trace=trace, debug=debug)",
...
def _camelize_identifier(name: str) -> str:
    ...
    cooked = [part[:1].upper() + part[1:] for part in parts if part]
    return "".join(cooked) or "Compiled"


class _WorldMethodCollector(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        func = node.func
        if isinstance(func, ast.Attribute) and isinstance(func.value, ast.Name) and func.value.id == self.root_name:
            self.methods.add(func.attr)
        self.generic_visit(node)


def collect_tool_names(program: CompiledProgram) -> tuple[str, ...]:
    names: set[str] = set()
    for role in program.roles.values():
        for phase in role.phases:
            names.update(phase.allowed_tools)
    return tuple(sorted(names))


def collect_host_world_methods(program: CompiledProgram) -> tuple[str, ...]:
    methods: set[str] = set()
    for step in iter_steps(program.workflow.steps):
        if isinstance(step, CompiledHostStep):
            source = textwrap.dedent(inspect.getsource(step.fn))
            tree = ast.parse(source)
            collector = _WorldMethodCollector(root_name="world")
            collector.visit(tree)
            methods.update(collector.methods)
    return tuple(sorted(methods))


def collect_world_methods(program: CompiledProgram) -> tuple[tuple[str, ...], tuple[str, ...], tuple[str, ...]]:
    host_methods = collect_host_world_methods(program)
    tool_methods = collect_tool_names(program)
    required = tuple(sorted(set(host_methods) | set(tool_methods)))
    return host_methods, tool_methods, required


def emit_world_adapter(program: CompiledProgram, *, adapter_class_name: str | None = None) -> str:
    adapter_class_name = adapter_class_name or f"{_camelize_identifier(program.name)}WorldAdapter"
    host_methods, tool_methods, required_methods = collect_world_methods(program)
    lines = [
        'from __future__ import annotations',
        '',
        f'"""Specialized world adapter emitted for {program.name}.',
        '',
        ...
        '',
        f'PROGRAM_NAME = {program.name!r}',
        f'HOST_WORLD_METHODS = {host_methods!r}',
        f'TOOL_WORLD_METHODS = {tool_methods!r}',
        f'REQUIRED_WORLD_METHODS = {required_methods!r}',
        '',
        ...
        f'        return f"{adapter_class_name}<{{type(self._backend).__name__}}>"',
        '',
    ]
    for method in required_methods:
        lines.extend([
            f'    def {method}(self, *args: Any, **kwargs: Any) -> Any:',
            f'        return self._backend.{method}(*args, **kwargs)',
            '',
        ])
    lines.extend([
        f'def adapt_world(world: Any) -> {adapter_class_name}:',
        f'    if isinstance(world, {adapter_class_name}):',
        '        return world',
        f'    return {adapter_class_name}(world)',
        '',
        '__all__ = [',
...
def _patch_kernel_for_bundle(kernel_source: str, *, adapter_module: str, adapter_class_name: str, package_relative: bool) -> str:
    import_stmt = f"from .{adapter_module} import {adapter_class_name}, adapt_world\n" if package_relative else f"from {adapter_module} import {adapter_class_name}, adapt_world\n"
    marker = 'import requests\n\n'
    if marker not in kernel_source:
        raise ValueError('Could not find import insertion point in emitted kernel.')
    kernel_source = kernel_source.replace(marker, marker + import_stmt + '\n', 1)
    target = ') -> dict[str, Any]:\n    state = dict(memory or {})'
    replacement = ') -> dict[str, Any]:\n    world = adapt_world(world)\n    state = dict(memory or {})'
    if target not in kernel_source:
        raise ValueError('Could not find workflow prologue in emitted kernel.')
    kernel_source = kernel_source.replace(target, replacement, 1)
    footer_target = '    "compare_traces",\n]'
    footer_replacement = '    "compare_traces",\n    "adapt_world",\n    "' + adapter_class_name + '",\n]'
    if footer_target in kernel_source:
        kernel_source = kernel_source.replace(footer_target, footer_replacement, 1)
    return kernel_source


def emit_python_module_bundle(program: CompiledProgram, *, package_name: str = "compiled_bundle", adapter_module_name: str = "world_adapter") -> dict[str, str]:
    adapter_class_name = f"{_camelize_identifier(program.name)}WorldAdapter"
    world_adapter_source = emit_world_adapter(program, adapter_class_name=adapter_class_name)
    compiled_kernel_source = emit_python_kernel(program)
    compiled_kernel_source = _patch_kernel_for_bundle(compiled_kernel_source, adapter_module=adapter_module_name, adapter_class_name=adapter_class_name, package_relative=True)
    init_source = (
        'from .compiled_kernel import KappaClient, compare_traces, describe_program, run_workflow, trace_to_json\n'
        f'from .{adapter_module_name} import {adapter_class_name}, HOST_WORLD_METHODS, TOOL_WORLD_METHODS, REQUIRED_WORLD_METHODS, adapt_world\n\n'
        '__all__ = [\n'
        '    "KappaClient",\n'
        '    "compare_traces",\n'
        '    "describe_program",\n'
    ...
    manifest_source = (
        ...
        f'WORLD_ADAPTER = {adapter_class_name!r}\n'
    )
    return {
        f"{package_name}/__init__.py": init_source,
        f"{package_name}/compiled_kernel.py": compiled_kernel_source,
        f"{package_name}/{adapter_module_name}.py": world_adapter_source,
        f"{package_name}/manifest.py": manifest_source,
    }


def write_python_module_bundle(program: CompiledProgram, output_dir: str, *, package_name: str = "compiled_bundle", adapter_module_name: str = "world_adapter") -> dict[str, str]:
    bundle = emit_python_module_bundle(program, package_name=package_name, adapter_module_name=adapter_module_name)
    root = Path(output_dir)
    for relative_path, source in bundle.items():
